...
public class RomeosGlockItem extends Item {
	public RomeosGlockItem(Item.Properties properties) {
	...
	public boolean releaseUsing(ItemStack itemstack, Level world, LivingEntity entity, int time) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				AdminBulletsEntity projectile = AdminBulletsEntity.shoot(world, entity, world.getRandom());
	...
	private ItemStack findAmmo(Player player) {
		ItemStack stack = ProjectileWeaponItem.getHeldProjectile(player, e -> e.getItem() == AdminBulletsEntity.PROJECTILE_ITEM.getItem());
		if (stack == ItemStack.EMPTY) {
			for (int i = 0; i < player.getInventory().items.size(); i++) {
				ItemStack teststack = player.getInventory().items.get(i);
				if (teststack != null && teststack.getItem() == AdminBulletsEntity.PROJECTILE_ITEM.getItem()) {
					stack = teststack;
					break;
				}
			}
